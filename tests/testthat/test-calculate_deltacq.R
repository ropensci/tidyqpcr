...
simulated_48_well_plate_plan <- create_blank_plate_96well() %>%
    ...
    mutate(target_id = rep(c("Target_1", "Target_2",
                             "Target_3", "Target_4"),
                           each = 12),
...
test_that("Unit test for the calculate_deltacq and calculate_deltadeltacq functions", {
    calculated_48_well_plate_with_deltacq <- calculate_deltacq_bysampleid(simulated_48_well_plate_with_cq %>%
                                                                              filter(prep_type == "+RT"), "Target_3") %>%
        arrange(well_row, well_col)

    calculated_48_well_plate_with_deltadeltacq_positive <- calculate_deltadeltacq_bytargetid(calculated_48_well_plate_with_deltacq,
                                                                                             "Sample_1") %>%
        arrange(well_row, well_col)

    calculated_48_well_plate_with_deltadeltacq_negative <- calculate_deltadeltacq_bytargetid(calculated_48_well_plate_with_deltacq,
                                                                                             "Sample_1", ddcq_positive = FALSE) %>%
        arrange(well_row, well_col)
