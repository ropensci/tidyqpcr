...
plate1plan <-
  ...
  mutate(sample_id = paste(BioRep, DilutionNice, sep = "_"))
```
...
plate1plan %>%
  filter(TechRep == "1", target_id == target_id_levels[1], DilutionNice == "1x")
...
ggplot(data = plates) +
  geom_point(aes(x = target_id, y = Cq, colour = DilutionNice, shape = Type),
...
est_efficiency_bytargetid(plates)
```
...
ggplot(
  data = filter(plates, Type == "+RT", target_id %in% target_id_levels_nice),
  aes(x = Dilution, y = Cq)
) +
  geom_point() +
  stat_smooth(
...
ggplot(
  data = platesmelt %>%
    filter(
      TechRep == "1", Type == "+RT", DilutionNice == "1x",
