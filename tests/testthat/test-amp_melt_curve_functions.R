...
simulated_amplification_curve_data <- tibble(well = rep(plate_wells,times = 15), 
                                             ...
                                             program_no = 2,
                                             fluor_raw = rep(c(10,15,13,20,20,50,60,70,90,130,170,280,440,670,1020), each = 16),
                                             fluor_base = rep(35, 240))

simulated_debased_amplification_curve_data <- simulated_amplification_curve_data %>%
    mutate(fluor_signal = rep(c(-25,-20,-22,-15,-15,15,25,35,55,95,135,245,405,635,985), each = 16))
...
simulated_dRdT_melt_curve_data_diff <- simulated_melt_curve_data_diff %>%
    mutate(dRdT = rep(c(-5,2,-7,0,-30,-10,-10,-20,-40,-40,-110,-160,-230,-350,NA), each = 16)) %>%
