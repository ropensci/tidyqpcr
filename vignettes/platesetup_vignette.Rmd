...
Each well of your plate measures one or more targets in one DNA/RNA sample. Each sample may be duplicated with different types of preparation (e.g. no reverse transcriptase enzyme controls). The minimal information you need to describe your plate is to specify the probe(s), sample, and preparation type for every well.  
...
We chose to name this variable "TargetID" to make it clear that this could refer to a primer set, primer set/ probe combination, detecting a single target sequence.
...
* All the information you need to identify the sample and probe/primer set *uniquely*.
