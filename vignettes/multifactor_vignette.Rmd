...
platesmed <- platesnorm %>%
  group_by(sample_id, Condition, biol_rep, HS, Drug, target_id) %>%
  summarize(
...
ggplot(data = platesmed) +
  geom_point(aes(x = target_id, y = Abund, shape = biol_rep, colour = Condition),
    position = position_jitter(width = 0.2, height = 0)
  ) +
  scale_shape_manual(values = c(15:18, 5:6)) +
...
ggplot(data = platesmed) +
  geom_point(aes(x = target_id, y = Abund, shape = biol_rep, colour = HS),
    position = position_jitter(width = 0.2, height = 0)
  ) +
...
ggplot(data = platesmed) +
  geom_point(aes(x = target_id, y = Abund, shape = biol_rep, colour = Drug),
    position = position_jitter(width = 0.2, height = 0)
  ) +
...
plate1curve <- read_lightcycler_1colour_raw(
  "../inst/extdata/Edward_qPCR_TxnInhibitors_HS_2018-06-15_plate1.txt"
  ) %>%
  debaseline() %>%
  left_join(plateplan) %>%
  mutate(biol_rep = 1, Plate = 1)

plate2curve <- read_lightcycler_1colour_raw(
  "../inst/extdata/Edward_qPCR_TxnInhibitors_HS_2018-06-15_plate2.txt"
  ) %>%
  debaseline() %>%
  left_join(plateplan) %>%
  mutate(biol_rep = 2, Plate = 2)

# platesamp  <- plate1curve %>% filter(program_no == 2)
