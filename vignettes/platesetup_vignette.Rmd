...
Functions including `display_plate` and `calculate_deltacq_bysampleid` assume that there is a column called `SampleID`, and use it to decide which wells get analysed together.
...
* In tidyqpcr we chose to give all variables capitalized (CamelCase) names, such as `SampleID`, because R functions are often lower-case, such as `sample`. Keeping variable names capitalized avoids some bugs and confusion.
