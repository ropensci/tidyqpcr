...
* Probe Calibration vignette, `vignette("calibration_vignette",package="tidyqpcr")`
...
Each well of your plate measures one or more probes in one DNA/RNA sample. Each sample may be duplicated with different types of preparation (e.g. no reverse transcriptase enzyme controls). The minimal information you need to describe your plate is to specify the probe(s), sample, and preparation type for every well.  
So, tidyqpcr expects that your plate plan has at a minimum three pieces of information per well: Probe, Sample, and Type.

**Probe** should uniquely identify a primer set or primer set/probe combination that you detect in a well. 
If you are detecting multiple regions of the same gene, or trialing multiple primer sets, you have to give them different Probe names.
Note again that the current (March 2020) version of tidyqpcr has been tested on SYBR/intercalating dye data with one probe per well only.

**Sample** should uniquely identify a nucleic acid sample in your experiment. Sample can either describe all the relevant information, for example `HeatShock_10min_RepA`, `InputControl_WildType_Rep3`; or provide other unique identifying information, for example `S013`. 
Functions including `display_plate` and `normalizeqPCR` assume that there is a column called `Sample`, and use it to decide which wells get analysed together.
...
Technical replicates are also necessary for qPCR experiments to track the variability. This occurs as multiple Wells, each of which has the same combination of Probe, Sample, and Type.
...
# Adding more samples and repeating probes
...
## Repeating probe names without repeating yourself
...
If we kept `colkey12` from above, then the variable `Sample` would no longer point uniquely to a single sample.
...
For example, suppose you are testing multiple probes against the same target, your favourite gene `YFG1`, and you have probe sets A, B, and C. Then you might want a variable called `Target` which lists `YFG1` for these, in addition to the variable `Probe` with values `YFG1_A`, `YFG1_B`, and `YFG1_C`.
...
* What if you are loading a 384-well plate with a fixed-spacing multichannel pipette that loads every second row? If you want to set up the plate plan to load the same Sample/Probe in two adjacent rows, using `rep(Sample,each=2)` might help.
* If your experiment spans multiple plates, it can be helpful to re-use `colkey` and `rowkey` in defining similar plate plans. You could re-use the entire plate plan if the plates are exact replicates, as long as you ensure the Sample name  includes replicate information and so is unique for the analysis.
