...
plate1 <- read_lightcycler_1colour_cq(
  "../inst/extdata/Edward_qPCR_TxnInhibitors_HS_2018-06-15_plate1_Ct.txt"
  ) %>%
  left_join(plateplan) %>%
  mutate(biol_rep = "1", plate = "1")


plate2 <- read_lightcycler_1colour_cq(
  "../inst/extdata/Edward_qPCR_TxnInhibitors_HS_2018-06-15_plate2_Ct.txt"
  ) %>%
  left_join(plateplan) %>%
...
ggplot(data = platesmed) +
  geom_point(aes(x = target_id, y = Abund, shape = biol_rep, colour = heat),
    position = position_jitter(width = 0.2, height = 0)
  ) +
